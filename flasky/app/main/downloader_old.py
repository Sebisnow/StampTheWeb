...
def get_url_history(url):
    ...
    try:
        ...
        html = soup.find('html')
        if html.has_attr('amp') or html.has_attr('⚡'):
            # URL is AMP-version
            # TODO Findout canonical URL to store in DB
            load_zip_submit(url, soup, encoding)
        else:
            # Check if link to AMP-version is present
            node = soup.find('link', {"rel": "amphtml"})
            if node is not None:
                res = requests.get(node['href'])
                if res.status_code >= 300:
                    flash(u'100 Bad URL Could not retrieve AMP URL to create timestamp for it.'+ url, 'error')
                    app.logger.error('100 Bad URL Could not retrieve AMP URL to create timestamp for it.'+url)
                    return
                load_zip_submit(url, BeautifulSoup(res.text.encode(encoding), 'html.parser'), encoding)
            else:
                sha256, doc = calculate_hash_for_html_doc(res.text)
                #if check_database_for_hash(sha256) < 1:
                originStampResult = save_render_zip_submit(doc, sha256, url, soup.title.string)
    #except RequestError:
        #errorCaught = '100', 'Bad URL', 'Could not retrieve URL to create timestamp for it.'
        #return ReturnResults(originStampResult, sha256, soup.title.string,errorCaught)
    #except DownloadError:
        #errorCaught = '200', 'Download', 'Could not create picture for URL'
        #return ReturnResults(originStampResult, sha256, soup.title.string,errorCaught)
    #except OriginstampError:
        #errorCaught = '300', 'Originstamp Error', 'Could not submit hash to originstamp'
        #return ReturnResults(originStampResult, sha256, soup.title.string,errorCaught)
    except:
        sha256=None
        ...
        if OriginstampError is not None:
            return ReturnResults(originStampResult, sha256, soup.title.string)
        else:
            return ReturnResults(None, sha256, soup.title.string)
