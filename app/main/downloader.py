...
def save_file_ipfs(text):
    ...
    app.logger.info(ipfs_hash[0]['Hash'])
...
def get_links_for_threads(joined_threads, proxy_list, num_threads, robot_check, user):
    ...
    for thread in joined_threads:
        ...
        if thread.ipfs_hash not in thread_dict:
            ...
            for link in links:
                app.logger.info("All links of thread-{}:\n{}".format(thread.threadID, links))
                link_threads = list()
                cnt *= 10
                ...
                started_threads = start_threads(None, link_threads, link, cnt, num_threads-1, robot_check,
                                                proxy_list)
                app.logger.info("Started Threads: {}".format(started_threads))
    ...
    for hash_value, link_dict in thread_dict.items():
        if hash_value != "error_threads":
            for link, thread_list in link_dict.items():
                app.logger.info("Going for the join of threads from {}: {}".format(link, thread_list))
...
def distributed_timestamp(url, html=None, proxies=None, user="Bot", robot_check=False, num_threads=5, location=None):
    ...
    """
    app.logger.info("Distributed timestamping")
    ...
    app.logger.info("Distributed timestamp done, return results.")
...
def start_threads(proxies, threads, url, cnt, num_threads, robot_check, proxy_list):
    num_threads += cnt
    app.logger.info("Proxies: " + str(proxies))
...
def _join_threads(threads, original_present=False):
    ...
    for thread in threads:
        if type(thread) == list:
            app.logger.info("having subthreads {}".format(thread))
            for subthread in thread:
                app.logger.info("joining subthread {}".format(subthread))
                subthread.join()
...
def _check_threads(threads):
    ...
    app.logger.info("Joined Threads: {}".format(votes))
    ...
    for thread in results:
        ...
        if thread.error is not None:
            ...
            if thread.prox_loc is not None and thread.error == TimeoutException:
                    ...
                    db.session.commit()
                    app.logger.info("Updated {} block count to {}".format(thread.prox_loc, str(country.block_count)))
            ...
            app.logger.info("No error but originstamp result is not there -> something went really wrong in Thread-{}"
            ...
            app.logger.info("The content from Thread-{} from {} does not match the original!\n {}\n{}"
            ...
            app.logger.info("Adding Post for Thread-{} from country {} with Originstamp result: {}"
                            .format(thread.threadID, thread.prox_loc, str(thread.originstamp_result)))
            add_post_to_db(thread.url, thread.url, thread.title, thread.ipfs_hash,
                           thread.originstamp_result["created_at"], user=user)
        else:
            app.logger.info("None Error and no hash in Thread-{} from {} proxy {} hash {}"
                            .format(thread.threadID, thread.prox_loc, thread.proxy, thread.ipfs_hash))
            if thread.prox_loc not in countries:
                error_threads.append(thread)
            results.remove(thread)

            app.logger.info("Adding threads to db done. Error thread count is {} versus {} working threads."
...
def add_post_to_db(url, body, title, sha256, originstamp_time, user=None):
    ...
    app.logger.info("Query Adding one new post. Already exists: {}".format(already_exists))
    if already_exists is not None:
        app.logger.info("Increasing  count for: {}".format(already_exists.hashVal))
        already_exists.count += 1
        db.session.add(already_exists)
    else:
        app.logger.info("Make new post")
        if user is None:
            ...
            app.logger.info("We have a user:{}".format(user))
            post_new = Post(body=body, urlSite=url, hashVal=sha256, webTitl=title,
                            origStampTime=datetime.strptime(originstamp_time, "%Y-%m-%dT%H:%M:%S.%fZ"),
                            author_id=user.id)
            app.logger.info("Finished adding one new post, committing")
...
def check_user(user):
    ...
    app.logger.info("retrieving user {}".format(user))
